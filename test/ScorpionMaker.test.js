...
const ScorpionMaker = artifacts.require('ScorpionMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('ScorpionMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.scorpion = await ScorpionToken.new({ from: alice });
        await this.scorpion.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await ScorpionMaker.new(this.factory.address, bar, this.scorpion.address, this.weth.address);
        this.scorpionWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.scorpion.address)).logs[0].args.pair);
    ...
    it('should make SCORPs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.scorpionWETH.address, '10000000', { from: minter });
        await this.scorpion.transfer(this.scorpionWETH.address, '10000000', { from: minter });
        await this.scorpionWETH.mint(minter);
        ...
        // After calling convert, bar should have SCORP value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.scorpion.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for SCORP-ETH pair
        await this.scorpion.transfer(this.scorpionWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.scorpionWETH.address, '100000', { from: minter });
        await this.scorpionWETH.sync();
        await this.scorpion.transfer(this.scorpionWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.scorpionWETH.address, '10000000', { from: minter });
        await this.scorpionWETH.mint(minter);
        assert.equal((await this.scorpionWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.scorpion.address, this.weth.address);
        assert.equal((await this.scorpion.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.scorpionWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});