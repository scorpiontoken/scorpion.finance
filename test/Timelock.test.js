...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should not allow non-owner to do operation', async () => {
        await this.scorpion.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.scorpion.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.scorpion.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.scorpion.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.scorpion.owner()).valueOf(), carol);
    });

    it('should also work with EmperorScorpion', async () => {
        ...
        await this.escorpion.add('100', this.lp1.address, true);
        await this.escorpion.transferOwnership(this.timelock.address, { from: alice });
        const eta = (await time.latest()).add(time.duration.days(4));
        await this.timelock.queueTransaction(
            this.escorpion.address, '0', 'set(uint256,uint256,bool)',
        ...
        assert.equal((await this.escorpion.poolInfo('0')).valueOf().allocPoint, '200');
        assert.equal((await this.escorpion.totalAllocPoint()).valueOf(), '300');
